          ...
          <div className="flex items-center gap-4">
            <div className="relative">
              ...
              <input
                ...
                className="pl-10 pr-4 py-3 rounded-2xl border border-gray-200 focus:ring-2 focus:ring-orange-500 outline-none w-64"
              />
            </div>
          </div>
        </div>
              ...
              >
                {["Burger King", "McDonalds", "KFC"].map((r) => (
                  ...
                  >
                    <input
                      type="checkbox"
                      className="w-5 h-5 rounded-md accent-orange-600"
                    />
          ...
          <main className="flex-grow">
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6">
              {foodItems.map((item, i) => (
                <div
                  key={i}
                  className="group bg-white p-5 rounded-[2.5rem] border border-gray-100 hover:shadow-2xl hover:shadow-orange-500/10 transition-all duration-500 relative"
                >
                  {/* Sale Badge (Agar item sale par ho) */}
                  {item?.is_on_sale === 1 && (
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex flex-col">
                      {item?.is_on_sale === 1 ? (
                        <>
                          {/* Sale Price (Current / Main Price) */}
                          <span className="text-xl font-black text-orange-600">
                            Rs. {item?.sale_price}
                          </span>
                          {/* Regular Price (Cut/Line-through) */}
