...
import { usePlaceOrder } from "../../hooks/website/useOrder";
import type { PlaceOrderPayload } from "../../services/website/orderService";
...
export const CheckoutPage: React.FC = () => {
  ...
  const { data: checkoutData, isLoading, isError } = useCheckout(cartId);
  ...
  const restaurant = checkout?.restaurant;

  const cartItems = cart?.items || [];

  const subtotal = Number(cart?.summary?.subtotal || 0);

  const shippingFee = Number(cart?.summary?.delivery_fee || 0);
...
useEffect(() => {
  ...
  const totalSavings = cartItems.reduce((totalSaving: number, item: any) => {
    const regularPrice = Number(item?.food_item?.regular_price || 0);
    ...
    const saving =
      regularPrice > unitPrice ? (regularPrice - unitPrice) * quantity : 0;
  ...
  const { data: addressResponse, isPending: isLoadingAddresses } =
    useAddresses();
  ...
  const [deliveryMethod, setDeliveryMethod] = useState<"self_pickup" | "ship">(
    "self_pickup",
  );
  ...
  useEffect(() => {
    if (Array.isArray(savedAddresses) && savedAddresses.length > 0) {
      ...
      if (defaultAddr) {
        ...
        // IMPORTANT: only first address selected
        ...
        Object.keys(newAddress).forEach((key) => {
          clearedErrors[key] = validateField(key, (newAddress as any)[key]);
        });
  ...
  const handleSelectSavedAddress = (addr: any) => {
    ...
    // IMPORTANT: selected address is identified by ID
    ...
    Object.keys(selected).forEach((key) => {
      clearedErrors[key] = validateField(key, (selected as any)[key]);
    });
  ...
  const handleDeliveryMethodChange = (method: "self_pickup" | "ship") => {
    setDeliveryMethod(method);

    // Clear address validation errors
    // when user selects self_pickup.
    if (method === "self_pickup") {
      setErrors({});
      setIsAddressExpanded(false);
    }

    // Open address section when Ship is selected.
    if (method === "ship") {
  ...
  if (paymentMethod === "online" && !isOnlineEnabled) {
    ...
    placeOrder(payload, {
      onSuccess: (response) => {
        ...
        // Show success overlay on the same checkout page
        setShowOrderSuccess(true);

        // Redirect to order detail after 3 seconds
        setTimeout(() => {
          navigate(`/user/order/${response?.data?.id}`);
        }, 3000);
      },
              ...
              <div className="space-y-4">
                ...
                ============================================ */}
                  ...
                  >
                    <div className="flex items-center gap-3.5">
                      <input
                        ...
                        onChange={() =>
                          handleDeliveryMethodChange("self_pickup")
                        }
                  ...
                  ========================================== */}
                    ...
                    {deliveryMethod === "self_pickup" && (
                      <div className="mt-4 ml-7 p-4 rounded-xl bg-white border border-orange-100">
                ...
                ============================================ */}
                  ...
                  >
                    <div className="flex items-center gap-3.5">
                      ...
                      <div>
                        <p className="text-sm font-bold text-gray-950">Ship</p>
              ...
              {deliveryMethod === "ship" && (
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    >
                      <MapPin size={13} />
                      Select Address ({savedAddresses.length})
                    </button>
                  </div>
                ...
                ================================================== */}
                  ...
                  {isAddressExpanded && (
                    <div className="p-6 rounded-3xl bg-gray-50/90 border border-gray-200/80 space-y-4 transition-all shadow-inner">
                      ...
                      <div className="space-y-1">
                        <label className="text-[11px] font-bold text-gray-600 uppercase tracking-wider flex items-center gap-1">
                          <Map size={12} className="text-orange-600" />
                          Street Address
                        </label>
      ...
      Please contact the restaurant or try again later.
            ...
            ================================================== */}
              ...
              <button
                ...
                className="w-full bg-gradient-to-r from-gray-950 to-gray-900 text-white py-4 rounded-2xl font-black hover:from-orange-600 hover:to-orange-500 transition-all duration-300 shadow-xl shadow-gray-950/20 active:scale-95 cursor-pointer text-sm tracking-wide disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isPlacingOrder
                  ? "Placing Order..."
                  : paymentMethod === "cod"
              ...
              <div className="space-y-4 pb-6 border-b border-gray-100">
                {cartItems.length === 0 ? (
                  ...
                  cartItems.map((item: any) => {
                    ...
                    const imageUrl = foodItem?.image?.media_path
                      ? `${import.meta.env.VITE_API_BASE_URL}/storage/${foodItem.image.media_path}`
                      : no_image;

                    const regularPrice = Number(foodItem?.regular_price || 0);
                          ...
                          <div className="min-w-0">
                            ...
                            {itemSavings > 0 && (
                              <p className="text-xs text-orange-600 font-bold flex items-center gap-1 mt-0.5">
                                <Tag size={11} />
                                Saved Rs. {itemSavings.toFixed(2)}
                              </p>
                            )}
                          </div>
                        </div>
                        ...
                        <div className="text-right flex-shrink-0">
                          {regularPrice > unitPrice && (
                            <span className="text-xs text-gray-400 line-through block font-medium">
                              Rs. {(regularPrice * quantity).toFixed(2)}
                            </span>
                          )}
                          ...
                          {quantity > 1 && (
                            <span className="text-[10px] text-gray-400 block">
                              Rs. {unitPrice.toFixed(2)} × {quantity}
                            </span>
                          )}
                        </div>
                      </div>
            ...
            ================================================== */}
              ...
              <div className="space-y-3 pt-2 text-sm text-gray-500 font-medium">
                ...
                <div className="flex justify-between items-center">
                  ...
                  {deliveryMethod === "self_pickup" || shippingFee === 0 ? (
            ...
            ================================================== */}
              ...
              <div className="border-t border-dashed border-gray-200 pt-4 flex justify-between items-center">
                ...
                <span className="text-2xl font-black text-orange-600">
                  ...
                  {(
                    subtotal +
                    (deliveryMethod === "self_pickup" ? 0 : shippingFee)
                  ).toFixed(2)}
                </span>
              </div>
            ...
            >
              ...
              <div className="space-y-4">
                ...
                <p className="text-xs text-gray-500 font-medium">
                  Choose an address from your saved locations to automatically
...
======================================================= */}
      ...
      {showOrderSuccess && (
        <div className="fixed inset-0 z-[100] bg-black/40 backdrop-blur-md flex items-center justify-center px-4 transition-all duration-300">
          <div className="w-full max-w-md bg-white rounded-[2.5rem] p-8 sm:p-10 text-center shadow-[0_30px_90px_rgba(0,0,0,0.15)] border border-gray-100 transform animate-[scaleUp_0.35s_cubic-bezier(0.16,1,0.3,1)]">
            ...
            <div className="flex justify-center mb-6">
              <div className="relative">
                ...
                {/* Main Icon Container */}
                <div className="relative w-22 h-22 rounded-full bg-gradient-to-tr from-emerald-500 to-emerald-400 flex items-center justify-center shadow-lg shadow-emerald-500/30 animate-[bounce_1.5s_infinite]">
                  <CheckCircle
                    size={48}
                    strokeWidth={2.8}
                    className="text-white drop-shadow-md animate-[scaleUp_0.5s_ease-out]"
                  />
                </div>
              </div>
            </div>
            ...
            <div className="mt-7 flex justify-center">
              <div className="w-6 h-6 border-2 border-orange-100 border-t-orange-600 rounded-full animate-spin shadow-sm" />
            </div>
          </div>
        </div>
      )}
